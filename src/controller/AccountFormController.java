...
public class AccountFormController {
    ...
    private boolean updateUserBasic() {
        try {
            ...
            return bo.updateBasicInfo(new UserInfoDTO(
                    lblUserId.getText(),
                    ...
                    if (null != txtDrivingLicenseNo.getText()) {
                        if (Pattern.compile("^[A-z0-9 ,.]{2,}$").matcher(txtAddress.getText()).matches()) {
                            if (Pattern.compile("^0[0-9]{9}$").matcher(txtMobileNumber.getText()).matches()) {
                                if (dpDob.getValue().equals(null)) {
                                    if (cmbGender.getValue().equals(null)) {
    ...
    private boolean updateUserQA() {
        try {
            return bo.updateUserQA(new UserInfoDTO(
                    lblUserId.getText(),
                    cmbQ1.getValue(),
                    cmbQ2.getValue(),
                    cmbQ3.getValue(),
    ...
    private void loadDataToForm(String userId) {
        try {
            UserInfoDTO dto = bo.search(userId);
